...
public class BinarySearch {
  ...
  private <E extends Comparable> Optional<Integer> binarySearchRecursionHelper(E[] arr, E target,
      int i) {
    //base cases
    if (arr.length == 0) {
    ...
    if (target.compareTo(arr[i]) > 0) { //target > arr[i]
      return binarySearchRecursionHelper(Arrays.copyOfRange(arr, i, arr.length), target,
          i + (i / 2));
    }
